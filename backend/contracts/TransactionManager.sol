...
contract TransactionManager is Ownable {
    ...
    event TransactionProposed(uint256 indexed transactionId, address indexed seller, address indexed buyer, uint256 amount, uint256 price);
    event TransactionVerified(uint256 indexed transactionId, address indexed verifier, bool approved);
    ...
    function proposeTransaction(address _buyer, uint256 _amount, uint256 _price) public returns (uint256) {
        require(isSeller[msg.sender], "Only sellers can propose transactions");
        require(isBuyer[_buyer], "Buyer is not registered");
        require(electricityToken.balanceOf(msg.sender) >= _amount, "Seller has insufficient electricity units");
        ...
        emit TransactionProposed(transactionId, msg.sender, _buyer, _amount, _price);
        return transactionId;
    }

    // --- Government Functionality ---
    function verifyPayment(uint256 _transactionId, bool _approved) public {
        require(msg.sender == governmentVerifier, "Only the government verifier can verify payments");
        ElectricityTransaction storage tx = transactions[_transactionId];
        require(tx.status == TransactionStatus.Pending, "Transaction is not pending verification");
    ...
    function completeTransaction(uint256 _transactionId) public payable {
        require(isBuyer[msg.sender], "Only registered buyers can complete transactions");
        ElectricityTransaction storage tx = transactions[_transactionId];
        require(tx.buyer == msg.sender, "You are not the buyer for this transaction");
        require(tx.status == TransactionStatus.Approved, "Transaction not approved by government");
        require(msg.value == tx.amount * tx.price, "Incorrect payment amount");

        // Transfer electricity units from seller to buyer
        require(electricityToken.transferFrom(tx.seller, tx.buyer, tx.amount), "Electricity unit transfer failed");
